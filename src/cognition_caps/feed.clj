...
(defn atom-store [stats]
  ...
  (let [items (take *feed-entry-count* (data/get-items db (:db-queries stats) :date-added 'desc))
...
(defn atom-blog [stats]
  (let [entries (take *feed-entry-count* (data/get-blog db (:db-queries stats)))
...
(defn atom-all [stats]
  (let [items (take *feed-entry-count* (data/get-items db (:db-queries stats) :date-added 'desc))
        blog-entries (take *feed-entry-count* (data/get-blog db (:db-queries stats)))
